...
class DocsAndTemplatesTest(unittest.TestCase):
    ...
    def test_docs_describe_grill_first_then_yolo_usage(self) -> None:
        ...
        self.assertIn("draft or incomplete explicit artifacts return `continue_planning`", readme)
        self.assertIn("explicit `--spec`/`--plan` artifacts are draft/incomplete", skill)
        self.assertIn("draft or incomplete artifacts return `continue_planning`", required_inputs)
